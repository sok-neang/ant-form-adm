...
export function useReservedFinalResult() {
    ...
    if (params.search) {
      ...
      list = list.filter(
        (item) =>
          ...
          (item.raw?.student?.email && item.raw.student.email.toLowerCase().includes(q)) ||
          (item.raw?.student?.khName && item.raw.student.khName.toLowerCase().includes(q)) ||
          (item.raw?.student?.enName && item.raw.student.enName.toLowerCase().includes(q)) ||
          (item.raw?.email && item.raw.email.toLowerCase().includes(q)) ||
  ...
  const fetchSubmissions = async (params = {}) => {
    ...
    try {
      ...
      const response = await submissionService.getReservedFinalResult(queryParams);
      ...
      console.error("Error fetching reserved final results:", error);
    } finally {
      loading.value = false;
    }
  };
