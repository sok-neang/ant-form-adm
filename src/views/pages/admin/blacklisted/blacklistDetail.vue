<template>
  <div class="container-fluid shortlist-container">
    ...
    <div v-else class="d-flex flex-column gap-4">
      ...
      <!-- 2. REASON FOR BLACKLIST CARD (NEW SECTION) -->
      <div class="card border-0 rounded-4 shadow-sm bg-white p-4">
        <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
          ...
          <span v-if="blacklistedDate" class="text-muted small">
            <i class="bi bi-calendar-event me-1"></i>{{ blacklistedDate }}
          </span>
        </div>
          ...
          <div class="d-flex align-items-start gap-3">
            <div class="flex-grow-1">
              ...
              <p class="mb-0 text-dark lh-base fs-6">
                {{ blacklistReason || "មិនមានការបញ្ជាក់ពីមូលហេតុជាក់លាក់នោះទេ" }}
              </p>
              <div v-if="blacklistNote" class="mt-3 pt-3 border-top border-danger-subtle">
                <span class="fw-semibold text-secondary small d-block mb-1">កំណត់ចំណាំបន្ថែម (Note):</span>
                <span class="text-muted small">{{ blacklistNote }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="review-actions">
      ...
      <div class="d-flex align-items-center gap-2">
        <span class="badge bg-danger-subtle text-danger px-3 py-2 rounded-pill">
          <i class="bi bi-shield-slash me-1"></i>សិស្សនេះស្ថិតនៅក្នុងបញ្ជីខ្មៅ
        </span>
      </div>
    </div>
...
const blacklistReason = computed(() => {
  return (
    ...
    submission.value?.blacklist?.reason ||
...
const blacklistNote = computed(() => {
  return (
    submission.value?.blacklistNote ||
    submission.value?.note ||
    submission.value?.notes ||
    submission.value?.blacklist?.note ||
    ""
  );
});

const blacklistedDate = computed(() => {
  const d = submission.value?.blacklistedAt || submission.value?.updatedAt || submission.value?.createdAt;
  if (!d) return "";
  try {
    const dt = new Date(d);
